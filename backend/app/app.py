...
async def read_resume_wordcloud(category: str = ""):
    ...
    data['Skills'] = data['Skills'].apply(lambda x: [skill.replace(' ', '_') for skill in x])
...
async def read_skills(category: str = ""):
    ...
    skill_counts = Counter(skill for skills_list in data['Skills'] for skill in skills_list)
